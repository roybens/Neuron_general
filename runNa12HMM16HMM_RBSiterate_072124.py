...
root_path_out = './Plots/12HMM16HMM/16-RBSiterateCode_12hmmOptScan_072024'
...
for mutname,dict in changesna12_071824best.items():
        ...
        # for fac in (1,1):
                ...
                simtim = tf.Na12Model_TF(ais_nav12_fac=4*50,ais_nav16_fac=6*7,nav12=6*2*0.5,nav16=3.5*0.5*fac, somaK=1, KP=500*0.5, KT=1, 
                                                        ais_ca = 1,ais_Kca = 1, soma_na12=1*3*1.3, soma_na16=1, dend_nav12=1, node_na = 1,
                                                        na12name = 'na12_HMM_TEMP_PARAMS',mut_name = 'na12_HMM_TEMP_PARAMS',na12mechs = ['na12','na12mut'],
                                                        na16name = 'na16_HMM_TEMP_PARAMS',na16mut_name = 'na16_HMM_TEMP_PARAMS',na16mechs=['na16','na16mut'],params_folder = './params/',
                                                        plots_folder = f'{root_path_out}/3-{mutname}', pfx=f'WT_', update=True)
                simtim.wtvsmut_stim_dvdt(wt_Vm=wt_Vm1,wt_t=wt_t1,sim_config=sim_config_soma,vs_amp=[0.5], fnpre=f'3-{mutname}-nav16_{str(fac)}')#f'5-aisKca_fac-{str(fac)}')
