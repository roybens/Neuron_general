...
root_path_out = '/global/homes/t/tfenton/Neuron_general-2/Plots/12HMM16HH_TF/ManuscriptFigs/Fix1216distribution/Scans_newAIS_020724/na4377AIS_scanK'

if not os.path.exists(root_path_out):
        os.mkdir(root_path_out)


vals = [0.5,1,2,4]#[0.6,0.75,1.25,1.5] #[0.1,0.25,0.4] #[0.5,2,3]
for i in vals:
...
# for i12 in np.arange(5,8,1):
        ...
        wt_Vm1,wt_I1,wt_t1,wt_stim1 = sim.get_stim_raw_data(stim_amp = 0.5,dt=0.005,rec_extra=False,stim_dur=500, sim_config = sim_config_soma)
        #features_df = ef.get_features(sim=sim,mutTXT='WT_soma', mut_name = 'na12_HMM_TF100923')
        sim.plot_model_FI_Vs_dvdt(wt_Vm=wt_Vm1,wt_t=wt_t1,sim_config=sim_config_soma,vs_amp=[0.5], fnpre=f'somaK-{i}')#fnpre=f'{mutTXT}')
