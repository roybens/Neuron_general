...
import numpy as np
class Na12ModelHH_TF:
    def __init__(self,na12name = 'na12_TF2',mut_name= 'na12_R850P_5may',  na12mechs = ['na12','na12_mut'],na16name = 'na16_orig2', na16mechs = ['na16','na16_mut'], params_folder = './params/',
...
def scan_sec_na():
    for fac in np.arange(0.1,1,0.1):
        sim = Na12ModelHH_TF(soma_na16=fac,soma_na12=fac)
        ...
        """
        sim = Na12ModelHH_TF(soma_na12=fac)
        fig_volts,axs = plt.subplots(2,figsize=(cm_to_in(8),cm_to_in(15)))
        sim.plot_stim(axs = axs[0],stim_amp = 0.5,dt=0.005)
        plot_dvdt_from_volts(sim.volt_soma,sim.dt,axs[1])
        ...
        sim = Na12ModelHH_TF(node_na=fac)
        fig_volts,axs = plt.subplots(2,figsize=(cm_to_in(8),cm_to_in(15)))
        sim.plot_stim(axs = axs[0],stim_amp = 0.5,dt=0.005)
        plot_dvdt_from_volts(sim.volt_soma,sim.dt,axs[1])
...
def scan12_16():
    for i12 in np.arange(2,0.4,-0.5):
        for i16 in np.arange(2,0.4,-0.5):
            sim = Na12ModelHH_TF(nav12=i12, nav16=i16)
            #sim.make_wt()
            fig_volts,axs = plt.subplots(2,figsize=(cm_to_in(8),cm_to_in(15)))
            sim.plot_stim(axs = axs[0],stim_amp = 0.5,dt=0.005)
            plot_dvdt_from_volts(sim.volt_soma,sim.dt,axs[1])
...
def test_params():
    for i in range(1,9):
        ...
        sim = Na12ModelHH_TF(na16name = na16_name)
        sim.plot_currents()
        fig_volts,axs = plt.subplots(2,figsize=(cm_to_in(9.5),cm_to_in(15)))
        sim.plot_stim(axs = axs[0],stim_amp = 0.5,dt=0.005)
        plot_dvdt_from_volts(sim.volt_soma,sim.dt,axs[1])
...
def dvdt_all(al1 = 'na12_TF', al2= 'na12_R850P_5may', stim_amp = 0.5, Typ = None, stim_dur = 500): #stim_amp = 0.5 #nA
    sim = Na12ModelHH_TF(al1,al2)
    
    if al1 == al2 and al1 == 'na12_TF':
        Typ = 'WT'
    elif al1 == al2:
        Typ = 'Hom'
    else: 
...
def dvdt_all_plot(al1 = 'na12_TF', al2= 'na12_R850P_5may',stim_amp = 0.5, stim_dur = 500):
    volt = [[],[],[]]
    dvdts = [[],[],[]]
...
#sim = Na12ModelHH_TF('na12_orig1', 'na12_orig1')
#sim.plot_currents()
